...
Harbor 是运行在 ROCK 沙箱中的 Agent Benchmark 评测框架。用户通过 `rockcli` 管理沙箱，Harber 在沙箱内逐 Trial 执行 Agent → Verifier 流程，产物落盘到 `/data/logs/user-defined/`（平台启动）或 `/tmp/harbor/jobs/`（本地 `harbor run`）。
